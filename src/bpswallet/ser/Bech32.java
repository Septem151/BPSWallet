...
public final class Bech32 {
    ...
    public static String hexToBech32(String humanPart, int witVer, String scriptPubKey) {
        return segwitToBech32(humanPart, witVer, ByteUtil.hex2bytes(scriptPubKey));
    }

    /*---- Static functions for segregated witness addresses ----*/
    /**
     * Encodes the specified segregated witness output into a Bech32 address
     * string.
     *
     * @param humanPart the prefix given to the resulting string, which should
     * be a mnemonic for the cryptocurrency name; must be not {@code null}, must
     * have length in the range [1, 83], must have all characters in the ASCII
     * range [33, 126] but excluding uppercase characters
     * @param witVer the witness version number; must be in the range [0, 16]
     * @param witProg the raw witness program, without the length byte; must be
     * not {@code null}, must have length in the range [2, 40]
     * @return the Bech32 address of the specified segregated witness output;
     * the result is entirely ASCII, lacks uppercase, and at most 90 characters
     * long
     ...
     * preconditions, or the combination of humanPart and witProg would make the
     * result exceed 90 characters
     */
    ...
    /**
     * Encodes the specified human-readable part prefix plus the specified array
     * of 5-bit data into a Bech32 string.
     *
     * @param humanPart the prefix given to the resulting string, which should
     * be a mnemonic for the cryptocurrency name; must be not {@code null}, must
     * have length in the range [1, 83], must have all characters in the ASCII
     * range [33, 126] but excluding uppercase characters
     * @param data a non-{@code null} sequence of zero or more values, where
     * each value is a uint5
    ...
    /**
     * Decodes the specified Bech32 address string into a segregated witness
     * output. The result is a triple (human-readable part, witness version,
     * witness program).
     *
     * @param s the Bech32 string to decode, which must be either all-lowercase
     * or all-uppercase, and at most 90 characters long
     * @return a triple where index 0 is a {@code String} representing the
     * human-readable part (which obeys all the rules as stated in the encoder),
     * index 1 is an {@code Integer} representing the witness version (in the
     * range [0, 16]), and index 2 is a new {@code byte[]} containing the
     * witness program (whose length is in the range [2, 40]; the array contains
     * 8-bit data)
     * @throws NullPointerException if the string is {@code null}
     * @throws IllegalArgumentException if the string is too long, has mixed
     * case, lacks a separator, has an invalid human-readable part, has
     * non-base-32 characters in the data, lacks a full checksum, has an
     * incorrect checksum, has an invalid witness version, or has an invalid
     * length of witness program
     */
    public static Object[] bech32ToHex(String s) {
        ...
        return new Object[]{decoded[0], witVer, ByteUtil.hexify(witProg)};
    }

    /**
     * Decodes the specified Bech32 string into a human-readable part and an
     * array of 5-bit data.
     *
     * @param s the Bech32 string to decode, which must be either all-lowercase
     * or all-uppercase, and at most 90 characters long
     * @return a pair where index 0 is a {@code String} representing the
     * human-readable part (which obeys all the rules as stated in the encoder),
     * and index 1 is a new {@code byte[]} containing the 5-bit data (whose
     * length is in the range [0, 82])
     * @throws NullPointerException if the string is {@code null}
     * @throws IllegalArgumentException if the string is too long, has mixed
     * case, lacks a separator, has an invalid human-readable part, has
     * non-base-32 characters in the data, lacks a full checksum, or has an
     * incorrect checksum
     */
